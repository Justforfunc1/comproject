...
#define LPSYS_CONFIG lpsys::Config::getConfig()
#define LPCONFIG_LOAD(filename) LPSYS_CONFIG.load(filename)
#define LPCONFIG_GETSTR(section, key, d) LPSYS_CONFIG.getString(section, key, d)
#define LPCONFIG_GETINT(section, key, d) LPSYS_CONFIG.getInt(section, key, d)
...
/**
 * 解析配置文件,并将配置项以key-value的形式存储到内存中
 */
class Config {
public:
	Config();
