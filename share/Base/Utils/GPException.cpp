...
const GPChar* SyscallException::_name = "SyscallException";

GPString SyscallException::name() const
{
    return _name;
}

GPVOID SyscallException::print(std::ostream& os) const
{
    GPException::print(os);
    if(_error != 0)
    {
    }
}

GPException* SyscallException::clone() const
{
    return new SyscallException(*this);
}

GPVOID SyscallException::_throw() const
{
    throw *this;
}

GPInt32 SyscallException::error()
{
    return _error;
}
